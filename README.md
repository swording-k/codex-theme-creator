...
```text
http://127.0.0.1:56938/
```

你会看到本机主题列表，可以做这些事：

- 选择已有主题或公开预设；
- 调整背景模糊、背景压暗、新聊天透明度、任务页透明度；
- 打开或关闭 GT 赛道、雨林雨线、雪山光雾等动态效果；
- 改主题强调色；
- 保存成“我的主题”；
- 一键切换到 Codex Desktop。

这一步不需要你懂 GitHub，也不需要自己改 JSON。它是一个本地控制台，只修改你自己 Mac 上的 Codex 主题库。
...
首版完整支持 macOS Codex Desktop，并提供三种完整界面语言。Windows 会作为下一阶段运行时目标，而不是把 macOS 脚本硬套过去。
...
1. 只想用同款：打开 Theme Studio，选主题，保存成“我的主题”，点击启用。
2. 想微调：在 Theme Studio 里调模糊、压暗、透明度、动态和强调色。
3. 想自己创作：安装本仓库的 Skill，然后在 Codex 里用一句话或一张参考图让 Codex 生成、安装、验证、导出主题。
