...
assert.match(
  css,
  /data-dream-motion-profile="gt-broadcast"[\s\S]{0,180}data-dream-motion-rain="true"[\s\S]{0,180}#codex-dream-skin-chrome::before[\s\S]{0,500}animation:\s*dream-skin-gt-rain/,
  "The GT profile should render an animated rain layer only when rain is enabled.",
);
assert.match(
  css,
  /data-dream-motion-profile="gt-broadcast"[\s\S]{0,220}data-dream-motion-signal-lights="true"[\s\S]{0,180}\.dream-skin-orbit::before[\s\S]{0,500}animation:\s*dream-skin-gt-lights/,
  "The GT profile should render restrained animated start lights.",
);
assert.match(
  css,
  /data-dream-motion-profile="gt-broadcast"[\s\S]{0,220}data-dream-motion-telemetry="true"[\s\S]{0,180}#codex-dream-skin-chrome::after[\s\S]{0,500}animation:\s*dream-skin-gt-sweep/,
  "The GT profile should render a telemetry sweep only when telemetry is enabled.",
);
assert.match(css, /@keyframes\s+dream-skin-gt-rain/, "The GT rain animation must be defined.");
assert.match(css, /@keyframes\s+dream-skin-gt-lights/, "The GT light animation must be defined.");
assert.match(css, /@keyframes\s+dream-skin-gt-sweep/, "The GT telemetry animation must be defined.");
assert.match(
  css,
  /group\\\/home-suggestions button span[\s\S]{0,160}color:\s*var\(--ds-text\) !important;/,
  "Suggestion card labels must not retain stale native dark text tokens.",
);
assert.match(
  css,
...
assert.equal(defaults.attributes.get("data-dream-motion-profile"), "none");
assert.equal(defaults.attributes.get("data-dream-motion-rain"), "false");
assert.equal(defaults.attributes.get("data-dream-motion-signal-lights"), "false");
assert.equal(defaults.attributes.get("data-dream-motion-telemetry"), "false");
assert.equal(defaults.rootStyle.values.get("--dream-motion-intensity"), "0");
...
const gtMotion = createFixture({
  id: "gt-motion-contract",
  appearance: "dark",
...
vm.runInNewContext(gtMotion.payload, gtMotion.context);
assert.equal(gtMotion.attributes.get("data-dream-motion-profile"), "gt-broadcast");
assert.equal(gtMotion.attributes.get("data-dream-motion-rain"), "true");
assert.equal(gtMotion.attributes.get("data-dream-motion-signal-lights"), "true");
assert.equal(gtMotion.attributes.get("data-dream-motion-telemetry"), "true");
assert.equal(gtMotion.rootStyle.values.get("--dream-motion-intensity"), "0.65");

const invalidMotion = createFixture({
  id: "invalid-motion-contract",
  motion: { profile: "unknown-profile", intensity: 9, rain: true },
});
vm.runInNewContext(invalidMotion.payload, invalidMotion.context);
assert.equal(invalidMotion.attributes.get("data-dream-motion-profile"), "none");
assert.equal(invalidMotion.attributes.get("data-dream-motion-rain"), "false");
assert.equal(invalidMotion.rootStyle.values.get("--dream-motion-intensity"), "0");
...
assert.equal(gtMotion.window.__CODEX_DREAM_SKIN_STATE__.cleanup(), true);
assert.equal(gtMotion.attributes.has("data-dream-motion-profile"), false);
assert.equal(gtMotion.attributes.has("data-dream-motion-rain"), false);
assert.equal(gtMotion.rootStyle.values.has("--dream-motion-intensity"), false);
