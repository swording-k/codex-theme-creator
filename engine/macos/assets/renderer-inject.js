((cssText, artDataUrl, themeConfig) => {
  ...
  const applyMotion = (root) => {
    const profiles = new Set(["gt-broadcast", "rainforest", "alpine"]);
    const profile = profiles.has(MOTION.profile) ? MOTION.profile : "none";
    const configuredIntensity = typeof MOTION.intensity === "number" && Number.isFinite(MOTION.intensity)
      ? clamp(MOTION.intensity, 0, 1) : 0.65;
    const intensity = profile === "none" ? 0 : configuredIntensity;
    const defaults = {
      rain: profile === "gt-broadcast" || profile === "rainforest",
      signalLights: profile === "gt-broadcast",
      telemetry: profile === "gt-broadcast",
    };
    const enabled = (name) => profile !== "none" &&
      (typeof MOTION[name] === "boolean" ? MOTION[name] : defaults[name]);

    setAttribute(root, "data-dream-motion-profile", profile);
    setAttribute(root, "data-dream-motion-rain", enabled("rain") ? "true" : "false");
    setAttribute(root, "data-dream-motion-signal-lights", enabled("signalLights") ? "true" : "false");
    setAttribute(root, "data-dream-motion-telemetry", enabled("telemetry") ? "true" : "false");
